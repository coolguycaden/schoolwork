...
char * toDigits(char * word){
	...
	char * encodedWord = (char *) malloc(sizeof(char) * 25);
	...
	for(index = 0; index < strlen(word); index++){
		...
		//This increases the needed space for encodedWord
		//
		length += neededLength;
	...
	//This removes the extra hyphen at the end
	//of encodedWord
	encodedWord[strlen(encodedWord) - 1] = '\0';
	encodedWord = (char *) realloc(encodedWord, sizeof(char) * (length - 2));
...
void textonymFind(char * word){
	...
	char * newWord = (char *) malloc(80);
	
	while(fgets(newWord, 80, dict) != NULL){
		newWord[strlen(newWord) - 1] = '\0';
		if(strlen(newWord) == strlen(word)){
			int c = 0;
			bool matches = false;
			while(c < strlen(word)){
				char wordDigit = toDigit(tolower(word[c]))[0];	
				char dictDigit = toDigit(newWord[c])[0]; 	
				if(wordDigit == dictDigit){
					c++;
					printf("%s has %c,", newWord, dictDigit);
					matches = true;
				} else {
					matches = false;
					c = strlen(word); 
