...
void swapWithNext(Node * p){
	if(p->prev != nullptr && p->next != nullptr && p->next->next != nullptr){
		Node * tempSecondNode = p->next;
		p->next = tempSecondNode->next;
		tempSecondNode->prev = p->prev;
		tempSecondNode->prev->next = tempSecondNode;
		p->next->prev = p;
		p->prev = tempSecondNode;
		tempSecondNode->next = p;
	}
}
